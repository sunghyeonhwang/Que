...
> **✅ 완료(2026-07-10)**: gemini-2.5-flash 온디맨드 카드 구현·배포. 상세는 HANDOFF "E-10 분석 AI" 절.
...
### E-F · 교육/온보딩 레이어 (2026-07-08 재검토 — **최우선 보강**)

**재검토 핵심**: Que 사용자(일반 사원)는 **OKR·Gantt·마일스톤·의존성 등 PM 이론 지식이 얕다**. 피드백(일정vs캘린더·반복마일스톤 사용법·Gantt·분석AI)이 바로 이 지식 격차에서 나왔다. 지금 계획은 **기능 중심인데 교육이 얇다** → 기능만 주면 **안 쓰거나 오해**한다. 도움말은 화면 사용법 위주고 **개념 프라이머가 없음**(간트·OKR 언급 0, `/planning` 빈 상태도 건조). 그래서 교육을 **횡단 요건**으로 승격한다.

- **E-F1 · 도움말 개념 프라이머(신규 섹션, 필요=YES)**: `help-content.ts`에 "**개념 쉽게**" 섹션 신설 — 각 개념을 **왜(가치)→무엇→어떻게(화면)→예시** 순 평이한 우리말로: ① **일정 vs 캘린더**(E-5 흡수: /schedule 일정 ↔ /projects 캘린더뷰 구분) ② **마일스톤·반복 업무**(E-6 흡수: 왜 쓰나) ③ 프로젝트-작업-마일스톤 관계 ④ (E-9 배포 시) **의존성·일정 리스크란**(선행 작업이 뭐고 왜 연결하나) ⑤ (todo_griff 배포 시) **OKR이란**. effort M.
- **E-F2 · In-context 가이드(신규 요건, 계획에 없던 것)**: 사람들은 도움말을 잘 안 봄 → 복잡한 화면에 안내를 심는다. **교육형 빈 상태**(예: `/planning` 첫 방문 "마일스톤이란? 프로젝트의 중요한 기한 지점…" + 예시 — 현 `milestone-list.tsx:32` "등록된 마일스톤이 없습니다"를 대체) + 어려운 라벨 옆 **(?) 도움 아이콘/툴팁**(선행 작업·일정 리스크·OKR/KR) + 쉬운 우리말 라벨. effort M(화면별 분산).
- **E-9/D-4에 교육 요건 필수 첨부**: **Gantt·OKR은 in-context 교육 없이는 무용지물.** E-9는 "PM 전문가 도구"가 아니라 **"일정이 밀리는 게 눈에 보이는 그림"**으로 포지셔닝(이미 '선행 작업'·'일정 리스크' 평이한 라벨 채택 — 유지). E-9b 프리뷰 시 **"팀이 실제로 이해하고 쓰는가" 검증**을 게이트로. D-4 OKR도 앱 내 첫 사용 가이드 필수.
- **E-10 분석 AI = 이 니즈의 실질 해결책 중 하나**: 지식 얕은 팀에 AI가 데이터를 **해석·설명·해결 방향 제시**(코치 역할)하는 게 정확히 맞음 → E-10의 가치가 이 관점에서 더 커짐(단 대상은 관리자·대표 유지).

**CLI·MCP 업데이트 필요? → 이 관점에서는 불필요(확인 완료)**: CLI/MCP는 **파워유저(개발·AI 연동) 표면**이라 일반 사원의 PM-지식 교육 니즈와 **직교**한다. 이미 `/tools` 사전준비 온보딩 + 도움말 CLI/MCP 섹션 존재, 계획된 E-4(명령어별 복사)·E-7(PAT)로 UX는 충분. 지식 격차는 **CLI/MCP가 아니라 E-F(도움말·in-context) + E-10(AI 코치)**가 채운다. → **CLI/MCP 신규 작업 없음.**
