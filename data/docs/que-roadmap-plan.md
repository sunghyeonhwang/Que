...
| --- | --- | --- | --- |
| **A-4 Sentry** | 중간 | 코드 가능, 활성엔 DSN 필요 | Sentry DSN(+선택 auth token) |
| **B-1 Slack Webhook** | 중간 | 코드 가능, 활성엔 URL 필요 | Slack Incoming Webhook URL |
| **B-2 Cron 전환** | 작음~중간 | **✅ 크레덴셜 없이 즉시 가능** | 없음(Cron secret은 env) |
| **B-3 실 PAT + 셀프 발급** | 중간 | 코드 가능 | 발급 토큰 팀 전달(운영) |
| **C-1 Google Calendar OAuth** | 중간 | 코드 골격 완성 | Google OAuth 클라이언트·동의화면 |
| **C-2 Slack 봇(2단계)** | 큼 | B-1 이후 | Slack 앱·봇 토큰·서명 시크릿 |
| **C-3 알림 설정/허브** | 중간 | 프리뷰 먼저 | 디자인 프리뷰(CLAUDE.md 규칙) |
| **C-4 마일스톤/반복 재연결** | 큼 | 화면 결정 먼저 | 화면/범위 결정 |

> **추천 다음 착수: B-2(Cron)** — 크레덴셜 불필요, 자립적. 그다음 크레덴셜 받는 대로 A-4·B-1.
