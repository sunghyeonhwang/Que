...
**1순위 — 웹에서 직접**: 완성된 YAML을 Que 웹 **일정 임포트(`/import`, 관리자 전용)** 화면에 붙여넣는다.
미리보기(생성될 항목·중복 스킵·차단 오류·미해결 질문)를 확인한 뒤 [등록]하면 클라이언트 →
프로젝트 → 마일스톤 → 작업(선행 연결) → 회의 순으로 일괄 생성된다(ChangeLog 기록).

**2순위 — Que 세션(Claude Code)에 붙여넣기**:
