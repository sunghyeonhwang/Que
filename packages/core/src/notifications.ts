 ...
 *  payment_created는 결제 요청 등록 즉시 **active 관리자 전원**(등록자가 관리자면 본인 포함 — 2026-07-14 확정) 개인 DM — recipient=관리자 userId,
...
export function dedupKeyFor(intent: NotificationIntent): string {
  ...
  // payment_done은 완료 이벤트(lastChangedAt ISO=marker)·수신자당 1회 — recipient(등록자/관리자)로 개별화,
