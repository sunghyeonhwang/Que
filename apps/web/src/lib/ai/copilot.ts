...
- **작업을 만들 때 어느 프로젝트에 속하는지 불명확하면, propose 전에 사용자에게 물어라** — get_project_status나 search_items로 활성 프로젝트 후보를 찾아 "어느 프로젝트에 넣을까요? (후보: A, B, …)" 형태로 제시한다. 사용자가 "멘딕스"처럼 단서를 줬으면 그 이름으로 프로젝트를 해석해 projectId를 채운다. 프로젝트 없이 만들어달라고 하면 그때만 projectId를 비운다.
