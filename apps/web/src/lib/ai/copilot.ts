...
export interface CopilotReply {
  ...
  /** 쓰기 intent면 확인 카드용 draft(실행 전). 없으면 조회 답변만. */
  draft?: CopilotDraft;
  /** draft의 id들을 이름으로 해석한 라벨(확인 카드 표기용). */
...
- 도구를 부르지 않았거나 결과가 비었으면, 추측하지 말고 "해당 데이터를 찾지 못했습니다"라고 답한다.
...
- 쓰기 대상(작업/프로젝트/사람)의 ID를 모르면 먼저 search_items나 get_* 도구로 찾은 뒤 propose_* 를 호출한다. 그래도 모르면 ID 필드를 비우고 제안한다(사용자가 카드에서 지정).
...
export async function runCopilot(
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    // 쓰기 제안(propose_*)이 있으면 첫 번째를 draft로 캡처하고 종료한다(실행하지 않음).
    const proposeCall = calls.find((p) => p.functionCall.name in PROPOSE_TO_KIND);
    if (proposeCall) {
      const draft = buildDraft(proposeCall.functionCall.name, proposeCall.functionCall.args);
      const draftLabels = draft ? await resolveDraftLabels(user, draft) : undefined;
      const text =
        collectText(parts) ||
        (draft
          ? "아래 내용을 준비했습니다. 확인 후 실행하세요."
          : "요청을 이해했지만 확인 카드를 만들 정보가 부족합니다. 조금 더 구체적으로 알려주세요.");
      return { text, sources: dedupeSources(sourceMap), draft: draft ?? undefined, draftLabels };
    }
