        ...
        {activities.length === 0 ? (
          ...
          <ul className="flex flex-col gap-4">
            {activities.map((activity) => (
              <li key={activity.id}>
                <Link
                  href={`/now?task=${activity.taskId}`}
                  className="-mx-2 flex items-start gap-3 rounded-lg px-2 py-1.5 transition-colors hover:bg-[var(--que-bg-muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
