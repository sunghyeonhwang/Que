...
const MENU_MAP: { group: string; menu: ReactNode; when: string }[] = [
  {
    ...
    menu: (
      <>
        <b>홈</b> · <b>데일리</b> · <b>작업 목록</b>
      </>
    ),
    when: "아침 조망 → 10시 체크인 → 내 할 일 (AI 질문은 상단바 Copilot 버튼 ⌘K)",
  },
  {
    group: "실행",
    menu: (
      <>
        <b>프로젝트</b>(간트·칸반) · <b>일정</b> · <b>회의록</b>
      </>
    ),
    when: "상태 옮길 때 · 일정 볼 때 · 회의 후",
  },
  {
    group: "운영",
    menu: (
      <>
        <b>Now</b> · <b>팀 현황</b> · <b>반복·마일스톤</b>
      </>
    ),
    when: "운영표(관리자) · 팀 상태 조망 · 기한 약속 관리",
  },
  {
    group: "참조",
    menu: (
  ...
  },
  {
    ...
    menu: (
      <>
        <b>결제요청</b> · <b>수정사항</b> · <b>설정</b> · <b>도움말</b> · <b>설계 FAQ</b>
      </>
    ),
  ...
  },
  {
    ...
    when: "사이드바 하단 바로가기 — 외부 화면, 새 탭으로 열림",
  },
];
...
const TOOLS: {
...
}[] = [
  {
    ...
    how: "10시 알림 → 폼 또는 AI 대화로 포커스 한마디. 어제·오늘 작업은 자동으로 붙습니다. 탭은 오늘·OKR·회고 3개.",
    win: "회의 없는 아침. 막힘 카드엔 ‘도울게요’ 한 번으로 도움이 당일 갑니다. 11시엔 AI 팀 요약.",
  ...
  },
  {
    ...
    how: "아침에 열어 확인 → 끝나면 체크 → 새 일은 자연어로(“금요일까지 배너 시안”). 밀린 일은 ‘내일로’ 한 번으로.",
  ...
  },
  {
    ...
    how: "4열 칸반 드래그(홀드·문제는 사유 필수), 실수는 [실행 취소]. 간트(기본)·보드·목록 전환. 프로젝트 달력은 일정 화면에서 봅니다.",
  ...
  },
  {
    ...
    how: "일·3일·주·2주·월 달력에 일정+작업+마일스톤 칩. 주말 숨김과 담당자·종류 필터는 다음 방문에도 기억됩니다.",
  ...
  },
  {
    ...
    how: "업로드하면 요약·할 일·결정이 자동으로 정리됩니다. 담당·기한 없는 할 일은 ‘확인 필요’로 남습니다.",
    win: "기록되지 않은 결정 0건. 회의가 실제 작업으로 이어지고, 결정 로그는 검색으로 다시 찾습니다.",
    copy: "“결정이 곧 기록이다.”",
  },
  {
    title: "반복 업무 · 마일스톤",
  ...
  },
  {
    ...
    how: "내게 관련된 신호만 모입니다. 아침 브리핑·스탠드업 버튼은 Slack DM으로 — 누르면 그 화면으로 바로. 종류별 수신은 설정 › 알림에서 조절합니다.",
      ...
      <div className="rounded-xl border border-[var(--que-border)] bg-[var(--que-brand-subtle)] p-5 md:p-[22px]">
        ...
        <ol className="mt-2 list-decimal space-y-1.5 pl-5 text-[var(--que-text)]">
          <li>
            ...
            <b>아이디 기억하기</b>를 체크해 두면 다음부터 바로 들어옵니다. 로그인 후{" "}
            <b>설정 › 보안</b>에서 패스키를 등록하면 다음부터 비밀번호 없이 들어옵니다.
          </li>
          <li>
            10시에{" "}
