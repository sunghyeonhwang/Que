...
**별도 레포 `~/Desktop/인터뷰/interview-app`**(interview.griff.co.kr — Que Supabase 공유·iv_ 접두, 원래 타 세션 소관이었으나 이번 요청은 이 세션에서 수행). 템플릿 개념 신설: `src/lib/templates.ts` 정본(TEMPLATES 배열 — 확장 여지) + `POST /api/admin/questionnaires/from-template`(isAdmin 401·미존재 키 400·duplicate insert 패턴) + Dashboard [템플릿에서 만들기]. **1호 "브랜드 로고 개선(리뉴얼) 인터뷰"** — 5섹션 22문항(현재 진단(계기·애착도 스케일)·브랜드 이해·방향 4스케일(중립 양극 라벨)·범위("모르겠다—제안 보고" 선택지 포함)/유지 자산/사용처/금기·레퍼런스 호오·의사결정자·성공 기준. 필수 11). 글래도스 승인(iv_ 밖 무접촉·스키마 정확 일치, 개정 후보 기록: 예산 축은 대면 질문 전제로 의도적 제외 — 필요 시 2호 개정). 라이브: 사이트 정상·API 미인증 401. 참고: 이 레포 dev 서버가 포트 점유 중이었음(글래도스가 종료 후 빌드 — 타 세션 재개 시 dev 재기동 필요할 수 있음).
