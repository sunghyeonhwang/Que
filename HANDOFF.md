...
4. **🆕 Pro 계정으로 프로젝트 이전 (내일 오전, 2026-07-07 예정)** — 위 3번 cron `*/10` 활성화의 (A)안을 언블록한다. **현재**: 프로젝트 `que`가 Hobby 팀 `griff0120s-projects`에 링크됨(`.vercel/project.json` orgId `team_adk2R8uxbLjr0BeZ9yzjfZxC`, projectId `prj_FSGUPN9iXotjqSw5X5btx4dN0At9`). **대상 Pro 팀/프로젝트**: `griff-fde0dc32/que-web` (<https://vercel.com/griff-fde0dc32/que-web>). **이전 시 재설정 필요**: ① `.vercel/project.json` 재링크(`vercel link`로 새 org/project) ② env 변수 전량 재설정(`SUPABASE_URL`·`SUPABASE_SECRET_KEY`·`AUTH_SECRET`·`CRON_SECRET`·`QUE_DB=supabase` 등 — 값은 `data/.env` 참조) ③ **커스텀 도메인 재연결**(`que.griff.co.kr`·`view.griff.co.kr` — Cloudflare CNAME은 그대로여도 Vercel 새 프로젝트에 도메인 add + 소유 검증 필요) ④ Root=`apps/web`·리전 `icn1` 재설정 ⑤ git 연동 재설정. **이전 완료 후**: `vercel.json`에 `*/10` crons 복구 + `CRON_SECRET` 설정 + `QUE_CRON_ACTIVE=1` → cron 활성화. ⚠️ 이전 전까지 [[end-of-work-routine]] 메모의 "que/griff0120s 자동배포"는 유효, 이전 후 갱신 필요.
