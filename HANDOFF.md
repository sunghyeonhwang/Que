...
#### 📄 홈 화면 역할별 명세 작성 (2026-07-10)

C-3b(홈 정식 디자인, Figma 시안 대기)의 기초 자료로 **`data/docs/que-home-spec.md`** 작성 — 대표(전사 조망)/관리자(어디가 막혔나)/사원(내 하루) 3분기 홈의 정보 구조·카드별 데이터·권한 3중 방어·반응형·디자인 가이드(토큰·색 의미·재사용 컴포넌트)·개선 후보를 현행 구현 기준으로 정리. **Figma 시안은 이 명세의 정보 구조를 유지**하고, 카드 추가/삭제/순서 변경은 명세 개정(사용자 승인) 후 반영하는 절차를 명시.
