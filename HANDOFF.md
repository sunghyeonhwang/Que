...
- **1차 통합 선별(2026-07-11)**: Que에 먼저 합칠 묶음 = **OS-1 상태형 KR + OS-2a 실패 분류 + OS-2b 외부 변경 접수**("에이전시 OKR 3종 세트") — company-os-plan.md 부록 A~C에 구현 가능 수준 상세 기획 완료(state_checks jsonb·milestone_retros·change_requests + 24h SLA + crisis 인프라 재사용). 구현 착수는 사용자 지시 대기.
