...
#### 🎤 인터뷰 앱 질문지 템플릿 + 로고 개선 질문지 (2026-07-20 사용자)
**별도 레포 `~/Desktop/인터뷰/interview-app`**(interview.griff.co.kr — Que Supabase 공유·iv_ 접두, 원래 타 세션 소관이었으나 이번 요청은 이 세션에서 수행). 템플릿 개념 신설: `src/lib/templates.ts` 정본(TEMPLATES 배열 — 확장 여지) + `POST /api/admin/questionnaires/from-template`(isAdmin 401·미존재 키 400·duplicate insert 패턴) + Dashboard [템플릿에서 만들기]. **1호 "브랜드 로고 개선(리뉴얼) 인터뷰"**(5섹션 22문항)·**2호 "신규 브랜드 인터뷰"**(5섹션 23문항 — 사업 배경·이름 상태·고객/시장·브랜드 성격(사람 은유+방향 4스케일)·만들 것/사용처/색·금기·레퍼런스/진행, 2026-07-20 같은 날 추가. 공통 축 상수 추출로 두 템플릿 공유, 문항 수는 tsx 실구조 평가로 확정 계수) — 1호 상세: 5섹션 22문항(현재 진단(계기·애착도 스케일)·브랜드 이해·방향 4스케일(중립 양극 라벨)·범위("모르겠다—제안 보고" 선택지 포함)/유지 자산/사용처/금기·레퍼런스 호오·의사결정자·성공 기준. 필수 11). 글래도스 승인(iv_ 밖 무접촉·스키마 정확 일치, 개정 후보 기록: 예산 축은 대면 질문 전제로 의도적 제외 — 필요 시 2호 개정). 라이브: 사이트 정상·API 미인증 401. 참고: 이 레포 dev 서버가 포트 점유 중이었음(글래도스가 종료 후 빌드 — 타 세션 재개 시 dev 재기동 필요할 수 있음).
...
#### 📺 view.griff.co.kr 현황판 개선 3건 (2026-07-19 사용자 "1-2-3진행")
**공개 무인증 화면 — 프라이버시 경계가 설계 축**(기존 노출 범위=이름·일정 제목·완료율 밖 확대 금지). ⑴**헤더 신호 티커**(getViewSignals): 문제·홀드·오늘 마감 집계+D-7 이내 마일스톤 1개(risk 색), 전부 0이면 green "이상 없음". ⑵**시간대 인지형**: 스탠드업 스트립(영업일 09:30~11:30 — N/8 제출+아바타, 11시 후 AI 요약 1줄. **⚠️ 게이트 조건 이행: 요약 첫 문장이 [막힘 클러스터](개인 이름+사유)라 공개 노출 금지 — [어제→오늘 흐름] 섹션 첫 문장만 대괄호 머리글 파싱으로 추출, 섹션 없으면 미노출이 안전 기본값**) + 다음 영업일 미리보기(주말/18시후/빈 보드 → 중앙 카드: 마감 N·첫 회의(private="일정" 마스킹)·마일스톤 3). 시간대 판정 전부 서버 KST. ⑶**슬라이드쇼 페이지 2종**: `view=milestones`(D-day 스트립)·`view=risk`(문제/홀드 목록 — **사유·다음 액션·도움 대상 제외**, 0건이면 "현재 막힘 없음"), view-settings 토글(기본 off), 기존 단일 타이머 재사용. 전부 loadReadOnlyDb(익명 GET 무쓰기). 글래도스 조건부 승인→이행("마스킹 정책은 데이터가 아니라 경로마다" — AI 요약 경유 유출 차단). 라이브: 티커 "이상 없음"·일요일 미리보기 카드(7/20 월)·위험 보드 빈 상태 확인.
...
#### 🧭 제품 전체 개선 6트랙 (2026-07-17 사용자 "1번부터 6번까지 순서대로" — 3커밋·3배포)
**①알림 개인 설정**: 21종 분류 CHANNEL 7·MUTABLE 4(personal_digest·checkin_prompt·standup_open·standup_remind — "재촉은 꺼도 신호는 못 끈다" 원칙)·ALWAYS_ON 10. `notification_prefs` 테이블(**적용+RLS 완료**), enqueueAndSend 공통 필터(**fail-open** — prefs 실패 시 전량 통과, held 경로 포함 toSend 기준·dedup 무오염), 설정›알림 탭(토글 즉시 저장·저장/복원 라이브 E2E 확인). **②예상 소요 입력 UX**: core parse-task 확장(2시간·30분=0.5·반나절=4·종일=8·한/두 시간, "N시간 후·뒤" 시점 방어, hour는 시각 파싱 전·minute은 후 — 순서 협상 테스트 고정), 공유 폼(작업추가·QuickAdd·일정·PM 보드)에 [30분][1시간][2시간][4시간][하루] 칩+직접 입력, 확인 카드 프리필·편집값 우선, Copilot 스키마 additive. **③QA 스윕+문서 감사**: 신규 5화면(회고·결정·2주·작업목록·알림 설정) 768×1024·1366 스윕 — **블로커 0**(768 헤더 우측 잘림 의심 1건 경미 기록). planner 온보딩·FAQ 감사 → 스테일 13항목 수정(오해 2건: 온보딩 Copilot 사이드바 잔존·프로젝트 캘린더 뷰 서술 / help 프로젝트 장도 캘린더 잔존 부분 스테일이라 s14 섹션 재구성 포함 전수 정리 / FAQ 원칙 서술 q1~q13은 전부 현행 일치 판정). FAQ 신규 문항 4후보(왜 캘린더 없나 등)는 기획서 동반 필요라 미착수 기록. **④Copilot 도구 3**: get_decisions(getDecisionLog 위임·limit 1~30·원문 대조 권고)·get_weekly_retro(막힘 해소 단정 금지)·get_today_load(**시그니처에 타인 인자 자체가 없음** — 개인 평가 오용 구조적 차단). **⑤전역 검색 '결정' 그룹**: search-data 서버 조립에 getDecisionLog 부분 일치(Gavel·?tab=decisions 딥링크·클라 type-only), 라이브 E2E("랜야드"→결정 히트). **⑥잔가지 5**: 재정렬 연타(버튼은 기존 충족 — **드래그 경로** pending 가드 신규)·자연어 기간("A부터 B까지"/"A~B", detectDate 공유·한쪽 실패 단일 폴백, **core 테스트 311**)·DuePicker 열 때 프리셋 재계산(refreshKey 옵셔널 — 소비처 10곳 무수정)·resolveRp **프로덕션 RP 고정**(NODE_ENV 게이트 — Host 위조 다운그레이드 차단, 글래도스 권고 이행)·ScheduleKind type-only 단일화(값 상수는 calendar-data의 getDb 번들 유입 때문에 로컬 유지+사유 주석). 글래도스 3회 승인(묶음1·묶음2+3). 잔여 파킹 무변경(C-1·B2·개인 비번·Copilot pro 평가).

#### ✅ 작업 목록(/today) 개선 3건 (2026-07-17 사용자 "1-2-3 진행")
⑴**내일로 원탭** — 기존 `deferTaskToTomorrowAction`(DayWrap 하루 마감에만 있던 것)의 **+1달력일을 다음 영업일로 수정**(금→월·토일→월, 신규 `lib/business-day.ts` nextBusinessDay — 시:분 유지, daily 영업일 술어와 동일. 서버·버튼·DayWrap 토스트가 같은 startAt에 같은 함수라 자정 경계 어긋남 원천 불가) + **내 작업 표 행에 원탭 버튼**(담당자 본인·미완료·startAt 존재만 노출 — 서버 거부 조건 미노출 원칙, 최종 방어는 core canEditTask). 토스트에 실제 이동 날짜 병기("7/20(월)로 옮기고…"). ⑵**오늘 부하 합계** — TodayData.plannedHours(**하루 작업만 — 기간 작업은 dateKeyOfIso 비교로 제외**(충돌 제외 정책과 동일 식·과대 계상 방지), done 제외), "오늘 계획 약 N시간" 카드: 8h 초과 amber "조정을 권합니다"(red 금지), "(미입력 M건 제외)" 정직 병기, 전부 미입력이면 생략. 8h 임계는 core에 근무시간 개념 부재라 UI 하드코딩(글래도스: core 도입 시 이관 후보 기록). ⑶**정렬·그룹** — `?sort=priority`·`?group=project`(화이트리스트·기본값 URL 생략·tab/panel 보존), 클라 재구성(sortMyTasksForView·groupMyTasksForView, my-task-table sections 기반 재작성 — TaskDoneCircle 등 기존 셀 재사용). 후속 수정 1건: Select items 맵 누락으로 raw 값("priority") 노출 → SORT_ITEMS/GROUP_ITEMS 맵 적용(schedule-filter 선례). 글래도스 승인. 라이브: 프로젝트 그룹 소제목·우선순위 정렬·행별 내일로 버튼·Select 한국어 라벨 확인(부하 카드는 실데이터 estimatedHours 전부 미입력이라 생략 상태가 정상).

#### 📋 회의록 개선 6건 (2026-07-17 사용자 — 제안 전건 승인 "2-1-3 진행하고 나머지 4-5-6진행")
⑴**참석자 Slack DM** — 업로드 후처리에 3단계 추가: attendeeIds−업로더 ∩ **canViewMeetingNote**(restricted 방어)에게 제목·일시·AI 요약 앞 3불릿·딥링크 DM. 신규 kind `meeting_note`(21종 3자 정합, CHECK 마이그레이션 **적용 완료**), dedup `meeting_note:<noteId>:<recipient>` 평생 1회, allowlist 미적용(트랜잭셔널). ⑵**미처리 에이징** — ActionRowData.ageBusinessDays(needs_review·candidate, 영업일 산술은 daily의 businessDaysElapsed 공유), /action "N일째 미처리" amber 뱃지(≥3영업일만), `buildWeeklyAgenda`에 staleActions 절(3영업일↑ 건수+오래된 순 5건, 0건 생략 — 게시 창·dedup 무변경). 업로더 주 1회 DM은 범위 제외(주석 기록). ⑶**진행률 뱃지** — note-list "처리 M/N"(처리=created·ignored·held, 전량 처리 green). ⑷**결정 로그** — generateNoteSummary가 [요약]/[결정] 구분자 구조화 응답(파싱 실패=결정 0건 강등·요약 유지, 최대 10건), 신규 `meeting_note_decisions`(**적용+RLS 완료**, 도메인 밖 직조회, 재생성 시 delete 후 재삽입 — delete 성공·insert 실패 시 일시 소실 가능하나 재생성 복구, 글래도스 기록), `/meeting-notes?tab=decisions` 탭(프로젝트 필터 ?project=, "AI 추출 — 원문 대조" 안내, 조회 전용·업무 데이터 쓰기 없음). ⑸**본문 전문 검색** — `?q=`(제목+markdownBody+AI 요약, **canView 필터 후 적용** — 비공개 존재도 미노출), 검색창 300ms 디바운스·N건 일치·제목만 하이라이트(indexOf+React children — XSS 불가). ⑹**지난 회의 팔로업** — getPreviousNoteCarryover(같은 kind 직전 회의록, 자기 자신 제외, canView 불가면 null — 존재 노출 금지), /action 접이식 블록(기본 접힘). menu.ts 회의록 children 3개(회의록·확인필요·결정)+CLAUDE.md·도움말 정합. 글래도스 조건부 승인→이행(SQL 헤더 적용 상태 갱신 — "코드는 현재, 문서는 과거" 시차 지적). **라이브 E2E**: 재생성으로 실 회의록에서 결정 4건 추출·결정 탭 표시, "처리 4/44" 뱃지·검색창 확인. 에이징 뱃지·팔로업은 조건 미충족으로 현재 미표시가 정상(Action 생성 1영업일째·회의록 1건 — 조건 되면 자연 표시).

#### 🌅 데일리 스탠드업 개선 5건 (2026-07-17 사용자 — 제안 6건 중 "2-1-3·4·6 진행, 5는 백로그")
⑴**미제출 카드 부재 표시** — 신규 `lib/away.ts`(홈 computeAwayChip 이관·공유 — 단일 소스): `computeAbsenceByUser`는 **뷰어 무관 마스킹**(비공개→항상 "자리비움", 공개 company→제목, 팀 공개 Que 일정은 부재 아님). 10:40 재촉 DM은 부재자 **조립 단계 스킵**(아웃박스 미적재 → dedup 무오염, 복귀 후 재적재 가능). ⑵**연속 막힘 "N일째" 뱃지** — `computeBlockerStreaks`(영업일 역순·금→월 연속·14영업일 상한·blockedTaskIds 교집합 우선, daily-data와 AI 요약이 단일 계산기 공유), UI는 ≥2일만 amber 뱃지(red 금지 — 문제 상태와 혼동). AI 팀 요약 프롬프트에 연속막힘 주입. ⑶**[도울게요] 원탭** — `daily/help-actions.ts` offerHelpAction(인증·본인 거부·target 실존): taskId 있으면 core addTaskComment 일반 댓글+당사자 Slack DM, 없으면 DM만. **신규 kind `standup_help`**(core NotificationKind additive — payment 선례, outbox CHECK 마이그레이션 `add-standup-help-kind.sql` **프로덕션 적용 완료**, 20종 3자 정합), dedup `standup_help:<date>:<target>:<actor>` 평생 1회, allowlist 우회(트랜잭셔널 — 결제 선례). 성공 시 버튼 "돕기로 했습니다" 잠금. ⑷**주간 회고 탭** — `/daily?tab=retro`+`?week=YYYY-MM-DD`(형식 화이트리스트·미래 주 이동 불가), `lib/weekly-retro-data.ts` getWeeklyRetroData(조회 전용): 일자별 제출 현황·사람별 요약(포커스·완료·제출 N/5)·막힘 목록(**"이후 언급 없음"=해소 추정, 단정 금지**)·그 주 AI 요약 아코디언. 한계는 파일 헤더에 정직하게 명시. menu.ts 데일리 children 3탭+CLAUDE.md 정합. ⑸**Slack 팀 요약 멘션** — 막힘 보유자를 standup_entries에서 **결정적 산출**(AI 본문 파싱 금지), `<@U…>` 멘션·매핑 없으면 이름 폴백, **막힘 내용은 채널에 미포함**(멘션만). 글래도스 승인(마스킹 우회 0·dedup 무오염·kind 3자 정합). 라이브: 회고 탭 렌더·주 이동 확인(부재 칩·도울게요는 해당 데이터 발생 시 — 팀 실사용에서 자연 검증). **Phase 4(체크인 카드 KR 진척)는 백로그** — `que+/daily-standup-okr-plan.md` 로드맵에 상태 주석 기록(사용자 결정).

#### 📝 회의록 업로드 시 Action 추출·AI 요약 자동화 (2026-07-20 사용자 — "업로드하면 한번에 자동으로")
업로드/Plaud 가져오기 성공 직후 `postProcessUploadedNote`가 자동 실행: ⑴**Action 추출** — `extractionStatus==="pending"`일 때만 core `extractActionItems`(actor=업로더, via=web — **Task 자동 생성 없음 규칙 그대로**, 산출물은 확인 필요 스테이징) ⑵**AI 요약** — gemini flash(`lib/meeting-summary.ts` generateNoteSummary: 창작·담당자 추정 금지 프롬프트, 원문 40k 절단, 15s race 가드). **두 후처리 각각 try-catch 격리 — 실패해도 업로드는 성공**(토스트가 결과 반영 4분기). 저장은 신규 `meeting_note_summaries`(note_id PK·content·model·generated_at/by) — **도메인 스냅샷 밖 직조회**(meetingNoteSchema 무변경 = write-through 마이그레이션 순서 제약 회피가 설계 의도, PAT/webauthn 선례), **프로덕션 적용+RLS 완료(2026-07-20)**. AI 저장은 standup_team_summaries와 동일한 의도적 예외. **권한: 요약=원문 파생물이라 원문과 동일 등급** — 목록 조립은 canViewMeetingNote 통과 노트만 일괄 조회, `regenerateNoteSummaryAction`은 인증→canView→업로더·admin(DB 현재값). UI: 원문 Sheet에 AI 요약 섹션(**React children 삽입만 — md 렌더러 미사용, XSS 불가**), [AI 요약 생성]/[재생성](canEdit 게이트), 수동 [Action 추출] 버튼은 폴백 유지, 도움말 자동화+외부 AI(Gemini) 전송 고지. createMeetingMinutes(직접 작성)는 의도적 스킵(주석). 글래도스 승인(비공개 요약 유출 0·race resolve-only·하위호환). **라이브 E2E**: 기존 실 회의록(에픽 7/14)에 생성 실행 — 8불릿 요약 저장·표시·재생성 버튼 전환 확인. mock dev는 요약만 게이트 비활성(추출은 동작).

#### 🔐 패스키(WebAuthn) 로그인 (2026-07-16 사용자 확정 — "추가 옵션, 비밀번호 공존")
이메일+비밀번호와 공존하는 추가 로그인. **DB**: `webauthn_credentials`(credential id PK·public_key·counter·device_name)+`webauthn_login_nonces`(jti PK=원타임 토큰 재사용 방지) — 마이그레이션 `db/supabase/add-webauthn-credentials.sql` **프로덕션 적용+RLS 활성 완료(2026-07-16)**. ⚠️ 두 테이블은 core 스냅샷 밖(PAT 선례 — persist 편입 금지, admin 직조회만). **서버**: `lib/auth/webauthn.ts` — rpID prod=`griff.co.kr`(서브도메인 공용)/dev=localhost, **expectedOrigin 고정 허용 목록**(que·gant·view·localhost — host 파생 금지), challenge=jose HS256(AUTH_SECRET) httpOnly 쿠키 300s·검증 성공 여부 무관 즉시 삭제(일회용), supabase 게이트 501(mock dev 비활성). API 4개(`/api/webauthn/register|login/options|verify`): register 세션 필수+챌린지 userId-세션 일치(타인 계정 심기 불가), login/verify는 credential 조회→**active 검증**→서명·counter 검증→counter/last_used 갱신→원타임 토큰(60s·aud·jti). **Auth.js**: 두 번째 Credentials 프로바이더 `"passkey"` — 토큰 검증→jti 논스 INSERT(충돌=재사용 거부, 모든 DB 오류 fail-closed)→active·role 재로드. 기존 프로바이더·콜백 무수정, must_change_password는 세션 shape 동일이라 자동 적용(우회 없음). **UI**: 설정›보안 패스키 카드(등록[기기 이름]·목록·인라인 이름변경·삭제 Dialog, 미지원 브라우저 비활성, 취소 무시·501 안내), 로그인 화면 "패스키로 로그인" 버튼(`passkeyLoginAction` — 기존 loginAction과 동일 callbackUrl 화이트리스트, redirect 경로 동일), 멤버 상세 관리자 패스키 제거 카드(분실 대응, 서버 admin 재확인), 도움말 반영. `webauthn-client.ts`(판별 유니온 ok|cancelled|disabled|error, hydration-safe 지원 감지 훅). deps: @simplewebauthn/server·browser, jose. 글래도스 승인(보안 전수 — 인증 체인 우회 0·챌린지/토큰 재사용 불가·fail-closed. 경미 기록: resolveRp의 localhost 분기를 prod에서 env로 비활성 권고 — 악용해도 자기 계정 localhost 크리덴셜 자해뿐). 라이브: login/options rpId 정상·미인증 register 401·설정 카드 렌더. **실기기 등록 세리머니(Touch ID)는 사용자 확인 대기**. 마이그레이션 미적용/롤백 상태에도 기존 인증 무영향(우아한 실패). 참고: Supabase 어드바이저리가 기존 테이블 11개 RLS 미설정 경고 — 신규 2개만 잠갔고 나머지는 사용자 결정 대기(아래 세션 노트).

#### 🗓 일정 화면 필터 확장 + 프로젝트 캘린더 탭 폐기 (2026-07-15 사용자 — "내 캘린더 vs 전체 회사 캘린더 차이")
사용자가 프로젝트 캘린더 탭과 /schedule의 역할 차이(전자=단일 프로젝트·마감만, 후자=팀 전체+회의·외부 캘린더)를 인지 → **프로젝트 캘린더 탭 제거하고 역량을 /schedule로 집중**. ⑴**프로젝트 '캘린더' 탭 제거**: view-tabs·project-view VIEW_KEYS `["list","board","gantt"]`, projects/page·projects-data에서 `getProjectCalendar`·`ProjectCalendar`·`CalendarCard`·`CalendarDay`·전용 헬퍼·고아 date-fns import 제거, `components/projects/calendar-view.tsx` **파일 삭제**. `?view=calendar` 구링크는 resolveView 기본 gantt로 폴백. ⑵**소유자 멤버별 다중 필터**(신규, 사용자 확정 "멤버별 다중선택"): `?owner=<id>,<id>`(콤마·실존 user id 화이트리스트), `filterScheduleItems`에 ownerIds Set — **ownerId 있는 task·event에만 적용, 마일스톤은 담당자 필드 부재라 제외**(hide 토글로만 제어). 필터 팝업 '담당자' 섹션=[전체]/[내 것만] 빠른 버튼+멤버 체크 리스트(아바타색·max-h-56 스크롤). ⑶**일정 종류 4종 표시 토글**(신규, 사용자 확정 "4종 개별"): `?hide=meeting,external`(4키 화이트리스트 task/meeting/external/milestone), CalendarViewItem에 additive `eventSource?: "que"|"company"`(회의=que·외부=company), milestone hide 시 page가 `milestones=[]`. 팝업 섹션 순서 표시→담당자→우선순위→키워드, activeCount·초기화가 owner·hide 포함. **기존 오늘 버튼·일간/3일/주간/월간 뷰·우선순위·키워드·키보드 단축키는 이미 있어 무변경**(사용자 요청 7건 중 5건이 기구현이었음). **후속(2026-07-16 사용자): 2주 보기 추가** — `range=2week`(이번 주 월요일부터 14일, week-calendar days 가변이라 상수 추가 수준), 드롭다운 일간·3일·주간·2주·월간, 이동 단위 2주, 키보드 `2`, 도움말 문구 3곳 갱신. 글래도스 승인·라이브 실측. **후속 2(같은 날, 사용자 "ㅇㅋ 추가" — 소형 4건)**: ⑴**주말 숨김 토글**(주간·2주 전용, `?weekend=hide`+쿠키 `que_schedule_weekend`, 렌더 days만 필터·조회 범위는 전체 유지로 주말 걸친 기간 작업 방어, 끌 때 `weekend=show` 명시 — 스테일 쿠키 우선 문제 회피) ⑵**미니 달력 날짜 점프**(신규 `date-jump.tsx`, due-picker KST 헬퍼 재사용, `?date=` set·타 파라미터 보존) ⑶**필터 기억**(담당자·표시만, 쿠키 `que_schedule_filters` JSON — 적용 시 기록·초기화 시 삭제, URL 우선·쿠키 값도 화이트리스트 재통과, 최종 적용값 `appliedOwner/appliedHide` prop으로 뱃지·폼 기준 전환) ⑷**월간 "+N개 더" 클릭→해당일 일간**(신규 `month-more-link.tsx`). 드래그 이동은 오조작 위험·비용으로 베타 피드백 후 결정(사용자 권고 수용). 글래도스 승인(layoutDay가 컬럼별 독립 배치라 주말 제거로 잘림/이중 렌더 구조적 불가 확인)·라이브 실측(주말 숨김 10열·날짜 점프 파라미터 보존). 글래도스 승인(마스킹 우회 0·화이트리스트·표시 필터라 권한 누수 아님·부제 카운트 필터 전 기준·클라 번들에 서버 모듈 유입 없음). 라이브 실측: 팝업 4체크+담당자 다중선택 렌더, '내 것만'→`?owner=hwang-sunghyeon` 반영 확인. 경미: ScheduleKind 4키가 calendar-data·schedule-filter 두 곳 중복 정의(클라 번들 격리 의도, 5번째 종류 추가 시 동기화 주의).
