...
3. Sentry DSN(에러 리포팅) · Slack 앱(알림/스탠드업) · CLI/MCP 배포(30번) · ~~스케줄러 Vercel Cron 전환~~ → **✅ B-2 완료(2026-07-04)**. `/api/cron/sync`(CRON_SECRET 인증) + `vercel.json` crons(10분). `db.ts` lazy는 `QUE_CRON_ACTIVE` 플래그로 제어(기본 유지, Cron 검증 후 `=1`로 끔). **운영: Vercel env `CRON_SECRET` 설정 + 플랜 10분 주기 확인(Hobby 불가·Pro 필요) + 확인 후 `QUE_CRON_ACTIVE=1`.** 남은 로드맵 상세는 `data/docs/que-roadmap-plan.md`.
